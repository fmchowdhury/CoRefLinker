 ...
 //  if(T_start!=NULL)printf("\n\n<%s>\n",T_start);fflush(stdout);
  ...
  while(T_start!=NULL && !ET){                      
          ...
          if(T_end == NULL){ 
             ...
             ET=1;
             if(T_end == NULL) {
               printf("ERROR: the End Tree |ET| marker is missing (line %ld)\n",example->docnum);
      ...
      *T_end = 0; // isolate the tree string
	  ...
	  if(PARAM_VECT == 1 && tree_kernel_params[tree_index].kernel_type==END_OF_TREE_KERNELS){
		  ...
		  PARAM_VECT = 0; // do not load from vector anymore
		}

      TKG_temp = TKGENERALITY; // save the command line TKGENERALITY
	  if(PARAM_VECT==1)TKGENERALITY=tree_kernel_params[tree_index].TKGENERALITY;
      
     // printf("Parse Tree in Memory:<");fflush(stdout);
     // writeTreeString(forest_vec_tmp[tree_index]->root);
     // printf(">\n\n");fflush(stdout);
    
      if(forest_vec_tmp[tree_index]->root!=NULL && TKGENERALITY!=STRING_KERNEL) // if it is not a string optimize tree kernel
      extractOrderedListofTreeNodes(forest_vec_tmp[tree_index]);
      else forest_vec_tmp[tree_index]->orderedNodeSet=NULL;

	  TKGENERALITY=TKG_temp; // restore the command line generality (altought it may not be ever used)      
      tree_index++;
      *pos = T_end - line + 4;
      T_start = T_end + 4;
...
*/				
	...
	while(*input==' ' || *input=='\t' )input++; //remove initial spaces

	if(strlen(input)==0) return NULL;

	if(strstr(input,"(")==NULL) { 
       printf("\n\nERROR: A tree with no initial left parenthesis: %s\n",input);
       fflush(stdout);
       exit(-1);
    }
  ...
  if(node != NULL){
   if(node->iNoOfChildren>0){
      ...
      free(node);
         ...
         free(node->sName);
   	     if(node->production!=NULL)free(node->production);
    	 free(node);
    }
  }
}
...
void freeForest(DOC *d){
    ...
    for(j=0; j<d->num_of_trees;j++)
	    if(d->forest_vec[j]->root != NULL){
         ...
         if(d->forest_vec[j]->orderedNodeSet!=NULL)free(d->forest_vec[j]->orderedNodeSet);
         free(d->forest_vec[j]);
        }
   free(d->forest_vec);
}
...
void fillTheNodeList(TreeNode *N, OrderedTreeNode *list, int *counter){
	...
	*production=0;
	if(N->iNoOfChildren>0){
       if(TKGENERALITY==PARTIAL_TREE_KERNEL || TKGENERALITY == NOLEAVES_PT_KERNEL)sprintf(production,"%s",N->sName); // only PT like kernels work on nodes 
       else sprintf(production,"%s->",N->sName);             // the other kernels works on productions

//	   printf("%s->",N->sName);
	   pre_term=1;
/* it considers also the non terminals */

	   for(i=0;i<N->iNoOfChildren;i++){
          if(TKGENERALITY!=PARTIAL_TREE_KERNEL && TKGENERALITY != NOLEAVES_PT_KERNEL){
	         strcat(production," ");  // add the right side production 
	         strcat(production,N->pChild[i]->sName);
          }
	     if(N->pChild[i]->iNoOfChildren>0) pre_term=0;
	     fillTheNodeList(N->pChild[i], list, counter);
...
//   	   printf("%s\n",production);
	...
	else if(TKGENERALITY >= BOW_SST_KERNEL) { //for Collins+bow and PT kernels add leaves in the production list 
               list[*counter].node=N;   
	           N->production=list[*counter].sName=strdup(N->sName);
	           N->pre_terminal=-1; // these nodes are not associated with a standard production 
	          (*counter)++;        // so they are terminal (less then pre-terminal)
         }
         else {N->pre_terminal=-1;N->production=NULL;list[*counter].node=NULL;}
	        
// printf("\nSingle Node: %s with children %d\n",N->production,N->iNoOfChildren);
}
