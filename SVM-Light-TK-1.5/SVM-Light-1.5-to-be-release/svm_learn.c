...
misclassified=0;
  for(i=0;i<totdoc;i++){
	  ...
	  if(!(i%100)){
			printf("\r Learning ETA: %2.2f%% - Elapsed Time (in Sec):  %.2f - SV: %ld",100*i/(float)totdoc,
...
//		misclassified++;
 ...
 /*  misclassified=0;
  for(i=0;i<totdoc;i++){
   ...
   printf("Iteration %d - Missclassified: %ld\n",k,misclassified);
}
...
printf("Kernel Cash: %d Mbs, Memory for one SV:%f Mbs\n\n",kernel_cache_size,occupancy);
...
for(k=0;k<learn_parm->svm_iter_to_shrink && misclassified ;k++) // use the value of shrinking param
{
  ...
  for(i=0;i<totdoc;i++){
   ...
   printf("\nIteration %d - Misclassified: %ld - Support Vectors: %ld\n",k,misclassified,model->sv_num-1);
   ...
   printf("\n\n\nFinished: Number of Support Vectors: %ld\n",model->sv_num-1);
