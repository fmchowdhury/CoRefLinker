       ...
       if(a->num_of_vectors == 0 || b->num_of_vectors == 0) return 0;
		...
		if(a->num_of_vectors <=i)
               ...
               exit(-1);
                 ...
                 else return basic_kernel_not_norm(kernel_parm, a, b,  i, j)/sqrt(a->vectors[i]->twonorm_STD*b->vectors[j]->twonorm_STD);
}
     ...
     /* calculate the kernel function */
{
  switch(kernel_parm->vectorial_approach_standard_kernel) {
   ...
   case 'S':/* non structured KERNEL Sequence k11+k22+k33+..+knn*/
    ...
    /* Start - added by Faisal on 11-May-2011 */
    case 'L': /* linear kernel */
            return(sprod(a, b));
...
double  
  ...
  if(kernel_parm->kernel_type==51 || kernel_parm->kernel_type==52){
   ...
   for(i=1; i< a->num_of_vectors-5 && i< b->num_of_vectors; i++)
     k1-=(basic_kernel(kernel_parm, a, b, 0, i)+basic_kernel(kernel_parm, a, b, i, 0));
  ...
  if(kernel_parm->kernel_type==53 || kernel_parm->kernel_type==52){
     ...
     for(i=1; i< a->num_of_trees-5 && i< b->num_of_trees; i++)
         k2-=(tree_kernel(kernel_parm, a, b, 0, i)+tree_kernel(kernel_parm, a, b, i, 0));
